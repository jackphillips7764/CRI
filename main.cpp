...
int main(int argc, char **argv) {
    ...
    if (setsockopt(master_socket, IPPROTO_IPV6, IPV6_V6ONLY, &opt,
                   sizeof(opt)) < 0) {
    ...
    int addr_len = sizeof(addr);
    getsockname(master_socket, (struct sockaddr *)&addr,
                (socklen_t *)&addr_len);
    ...
    while (1) {
        ...
        // call select
        int activity = select(max_sd + 1, &readfds, &writefds, 0,
                              0); /* TODO: might want to put actual timeout */
